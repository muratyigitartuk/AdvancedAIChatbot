...
class AIEngine:
    ...
    def process_input(self, user_id: int, conversation_id: int, message: str) -> Dict[str, Any]:
        ...
        metadata = self._analyze_message(message)
        
        # Update user topics based on extracted topics
        if metadata.get("topics"):
            self.user_profile_service.update_user_topics(user_id, metadata["topics"])
        ...
        if self.proactive_engine and self.proactive_engine.should_send_recommendation(user_id):
            ...
            if recommendations:
                proactive_msg = recommendations[0]["message"]
        
        # Save the conversation
        self.user_profile_service.add_user_message(user_id, conversation_id, message, metadata)
        self.user_profile_service.add_bot_message(
            conversation_id, 
            response, 
            {"generated_from": self.model}
        )
        
        return {
            "response": response,
            "metadata": metadata,
            "proactive_recommendation": proactive_msg
        }
    ...
    def _extract_entities(self, message: str) -> List[Dict[str, Any]]:
        ...
        for entity in entities:
            if current_entity and entity["entity"].startswith("I-") and current_entity["entity"] == entity["entity"].replace("I-", "B-"):
    ...
    def _extract_topics(self, message: str) -> List[str]:
        ...
        keywords = ["finance", "health", "technology", "travel", "food", 
                   "education", "sports", "entertainment", "news", "weather"]
    ...
    def _generate_response(self, context: str) -> str:
        ...
        try:
            ...
            response = openai.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that learns from user interactions."},
