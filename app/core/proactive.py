...
class ProactiveEngine:
    ...
    def generate_recommendations(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        for conv in history:
            for msg in conv["messages"]:
                if msg["is_user"] and msg["message_metadata"].get("topics"):
        ...
        if history:
            ...
            if last_conversation["messages"]:
                ...
                if not last_message["is_user"]:  # If last message was from bot
                    ...
                    if last_message["message_metadata"].get("topics") and "technology" in last_message["message_metadata"]["topics"]:
                        recommendations.append({
                            "type": "follow_up",
                            "topic": "technology",
    ...
    def should_send_recommendation(self, user_id: int) -> bool:
        ...
        if last_recommendation_time:
            ...
            # Determine if enough time has passed based on frequency setting
            if frequency == "low" and hours_since < 48:  # Every 2 days
                return False
            elif frequency == "medium" and hours_since < 24:  # Daily
                return False
            elif frequency == "high" and hours_since < 6:  # Several times per day
                return False
        
        # Update last recommendation time
        preferences["last_recommendation_time"] = now.isoformat()
        self.user_profile_service.update_user_preferences(user_id, preferences)
