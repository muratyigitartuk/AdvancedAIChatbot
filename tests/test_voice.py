...
This module contains tests for the voice processing functionality of the advanced AI chatbot application.
It tests various voice-related features including:
...
These tests use mocked voice services to simulate the behavior of external speech processing APIs,
ensuring that the application correctly handles voice data conversion and related error cases.
The tests validate both successful conversions and proper error handling for various scenarios.
"""
...
def test_tts_error_handling(mock_factory, client, test_user_token):
    ...
    data = response.json()
    assert "Error generating speech" in data["detail"]
